...
/* ST7789 8位8080并口屏使用标志
 * 0: 没有使用ST7789 8位8080并口屏
 * 1: 使用的是ST7789 8位8080并口屏
 */
...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
    ...
    if (g_spi_mode == 0)
    {
        ...
        if (st7789_bus_8bit == 1)   /* 8位8080并口读颜色数据 */
        {
            lcd_opt_delay(2);
            r = lcd_rd_data() & 0XFF;
            lcd_opt_delay(2);
...
void lcd_init(void)
{
    ...
    sys_gpio_set(LCD_RST_GPIO_PORT, LCD_RST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* LCD_RST引脚模式设置(推挽输出) */
    ...
    if (lcddev.id != 0X7796)    /* 不是7796,尝试看看是不是ST7789 */
    {
        ...
        else                            /* 尝试是不是ST7789 8位8080并口 */
        {
            ...
            {
                lcddev.id = 0x7789;
                st7789_bus_8bit = 1;    /* 使用ST7789 8位8080并口屏 */      
            }          
        }
    ...
    lcd_display_dir(LCD_DIR);    /* 屏幕设置方向 */
    ...
    for (i = 0; i < height; i++)
    {
        ...
        if (g_spi_mode == 0)
        {
            for (j = 0; j < width; j++)
            {
                if (st7789_bus_8bit == 1)
                {
                    LCD->LCD_RAM = color[i * width + j] >> 8;  /* 写入数据 */
                }
...
void DMA_Init_LCD(void)
{
	...
	hdma_lcd.Init.Request             = DMA_REQUEST_MEM2MEM;    // 存储器到存储器模式
    hdma_lcd.Init.Direction           = DMA_MEMORY_TO_MEMORY;       // 内存到内存传输
    hdma_lcd.Init.PeriphInc           = DMA_PINC_ENABLE;            // 外设地址（图形缓冲区）自增
    hdma_lcd.Init.MemInc              = DMA_MINC_DISABLE;           // LCD 地址不自增
    hdma_lcd.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;    // 外设数据宽度：16 位
    hdma_lcd.Init.MemDataAlignment    = DMA_MDATAALIGN_HALFWORD;    // 内存数据宽度：16 位
    hdma_lcd.Init.Mode                = DMA_NORMAL;                 // 普通模式
    hdma_lcd.Init.Priority            = DMA_PRIORITY_HIGH;          // 高优先级
	
//    hdma_lcd.Init.FIFOMode            = DMA_FIFOMODE_ENABLE;        // 关闭 FIFO
//    hdma_lcd.Init.FIFOThreshold       = DMA_FIFO_THRESHOLD_FULL;    // FIFO 阈值设置（无效）
//    hdma_lcd.Init.MemBurst            = DMA_MBURST_SINGLE;          // 单次传输
//    hdma_lcd.Init.PeriphBurst         = DMA_PBURST_SINGLE;          // 单次传输
	
	
	hdma_lcd.Init.FIFOMode            = DMA_FIFOMODE_ENABLE;       // 启用 FIFO
    hdma_lcd.Init.FIFOThreshold       = DMA_FIFO_THRESHOLD_FULL;   // FIFO 阈值设置为满
    hdma_lcd.Init.MemBurst            = DMA_MBURST_INC8;           // 内存突发传输：8个节拍
    hdma_lcd.Init.PeriphBurst         = DMA_PBURST_SINGLE;         // 外设突发传输：8个节拍单次
